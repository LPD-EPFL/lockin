...
#define LADAP_BASE_LOCK              32
#define LADAP_BASE_UNLOCK            2
...
#if LADAP_PRINT == 1
	printf("[lock train] #spin   lock try: (succ ratio: %6.4f)=> %-5zu (%p)\n",
	       succ_ratio, m->do_spins_lock, (void*) m);
#endif

#warning Are you sure you want to wake them all up?
	sys_futex(m, FUTEX_WAKE_PRIVATE, INT_MAX, NULL, NULL, 0);
      }
...
/* 	if (succ_ratio >= LADAP_SUCC_RATIO_MIN) */
/* 	  { */
/* 	    m->do_spins_unlock =  LADAP_BASE_UNLOCK << ((int) (10 * succ_ratio)); */
/* 	  } */
/* 	else */
/* 	  { */
...
/* 	  } */
/* #if LADAP_PRINT == 1 */
/* 	printf("[lock train] #spin unlock try: (succ ratio: %6.4f)=> %-5zu (%p)\n", */
/* 	       succ_ratio, m->do_spins_unlock, (void*) m); */
/* #endif */
/* 	/\* __sync_fetch_and_sub(&m->n_slow_rel, n_slow_rel); *\/ */
/* 	/\* __sync_fetch_and_sub(&m->n_spin_rel, n_spin_rel); *\/ */
