...
#define LADAP_SPIN_TRIES_LOCK        16384
#define LADAP_SPIN_TRIES_UNLOCK      32

#define LADAP_FUTEX_UNLOCK_EVERY     1023
/* #define LADAP_FUTEX_UNLOCK_EVERY     ((2L<<32)-1) */
...
#define CACHE_LINE_SIZE 64
  ...
  typedef struct ladap_lock
  {
    ...
    size_t n_spin_rel;
#if PADDING == 1
    uint8_t padding[CACHE_LINE_SIZE - 2 * sizeof(size_t)];
#endif
  } ladap_lock_t;
...
#define ladap_unlikely(x) __builtin_expect((x), 0)
  ...
  ladap_lock(ladap_lock_t* m)
  {
	...
	unsigned int i;
	/* Try to grab lock */
	for (i = 0; i < LADAP_SPIN_TRIES_LOCK; i++)
	  {
  ...
  ladap_unlock(ladap_lock_t* m)
  {
    ...
    for (i = 0; i < LADAP_SPIN_TRIES_UNLOCK; i++)
      {
	if (m->l.b.locked) 
	  {
	    size_t sr = m->n_spin_rel++; //__sync_add_and_fetch(&m->n_spin_rel, 1);
	    if ((sr & LADAP_FUTEX_UNLOCK_EVERY) == 0)
	      {
