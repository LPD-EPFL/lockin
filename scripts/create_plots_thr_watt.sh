...
do
	...
	do
	    ...
	    dat="$dat_folder/energy_all_locks.c$cl.w$rw.l$nl.dat";
	    gp="$gp_folder/energy_all_locks.c$cl.w$rw.l$nl.gp";
	    eps="$out_folder/energy_all_locks.c$cl.w$rw.l$nl.eps";
...
"$dat" using 1:(\$26) title  "MUTEX" ls 9 with linespoints 
...
"$dat" using 1:(\$28) title  "MUTEX" ls 9 with linespoints 

set title  "$title3"; 
set size 1.2,1.4
set origin 1.8,0;
set ylabel "Energy per Operation (mJoule/op)" offset 1.5
...
"$dat" using 1:(\$27) title  "MUTEX" ls 9 with linespoints 
...
EOF
	...
	name_prefix="energy_all_locks.c$cl.w$rw";
