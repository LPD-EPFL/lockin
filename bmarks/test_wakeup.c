...
main(int argc, char **argv)
{
  ...
  size_t n_lock = 0, n_unlock = 0, ticks_lock = 0, ticks_unlock = 0;
  for (i = 0; i < num_threads; i++) 
    {
      if (verbose)
	{
  ...
  printf("#avg sleep lo : %-10zu (%zu times) -- per thread: %zu\n", 
	 ticks_lock / n_lock, n_lock, ticks_lock / (n_lock * (num_threads - 1)));
  printf("#avg sleep un : %-10zu (%zu times)\n", ticks_unlock / n_unlock, n_lock);
