...
upmutex_lock_t lock = UPMUTEX_INITIALIZER;
...
static void
_lock(upmutex_lock_t* m, thread_data_t* d)
{
  volatile ticks s = getticks();
  sys_futex(&m->lock, FUTEX_WAIT_PRIVATE, 2, NULL, NULL, 0);
...
static void
_unlock(upmutex_lock_t* m, thread_data_t* d)
{
  volatile ticks s;
  do 
    {
  ...
  while((!sys_futex(&m->lock, FUTEX_WAKE_PRIVATE, 1, NULL, NULL, 0)) && futex_val == 2);
  volatile ticks e = getticks();
  d->n_unlock++;
  d->ticks_unlock += (e - s);
}
...
main(int argc, char **argv)
{
  ...
  n_unlock = (!n_unlock) ? 1 : n_unlock;
  n_cdelay = (!n_cdelay) ? 1 : n_cdelay;
  size_t lat_lock = ticks_lock / n_lock;
  size_t lat_unlock = ticks_unlock / n_unlock;
  size_t lat_cdelay = ticks_cdelay / n_cdelay;
  size_t lat_handover = (ticks_lock - ticks_cdelay) / n_lock;

  printf("#avg sleep lo : %-10zu (%zu times)\n", lat_lock, n_lock);
  printf("#avg handover : %-10zu\n", lat_handover);
  printf("#avg sleep un : %-10zu (%zu times)\n", lat_unlock, n_unlock);
  printf("#avg cdelay   : %-10zu (%zu times)\n", lat_cdelay, n_cdelay);
  size_t cs = 2 * n_lock;
  printf("#cs           ; %zu\n", cs);
  printf("#       %-10s %-10s %-10s %-10s\n", "lock", "handover", "unlock", "cdelay");
  printf("[STATS] %-10zu %-10zu %-10zu %-10zu\n", lat_lock, lat_handover, lat_unlock, lat_cdelay);
  /* rapl_stats_t s; */
  /* RR_STATS(&s); */
