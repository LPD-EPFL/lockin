...
#define MEASURE_EVERY 1023

static inline int
ttas_lock_lock(ttas_lock_t* lock, thread_data_t* d) 
{
  volatile ticks s = 0;
  if ((++d->num_acquires & MEASURE_EVERY) == 0)
    {
      s = getticks();
    }
  while (lock->lock != 0)
...
main(int argc, char **argv)
{
  ...
  size_t avg_lat = (ticks_lock / n_lock);
  printf("#acquires     : %10zu ( %10zu / s)\n", avg_lat, avg_lat);
  printf("#real thr     : %10lu ( %10.0f / s)\n", acquires, thr);
#if DELAY == DELAY_FAIR
