...
#define DELAY_FAIR      2
#define DELAY_FAIR_FUT  3
#define DELAY_FAIR_PLUS 4
#define DELAY_FAIR_FULL 5

#define DELAY               DELAY_FAIR
#define MEASURE_CONSEQUTIVE 0
...
#if DELAY >= DELAY_FAIR_PLUS || MEASURE_CONSEQUTIVE == 1
volatile size_t shared_counter = 0;
#endif
...
#endif
#if DELAY == DELAY_FAIR_FULL
  int counter_check = (num_threads > 1);
#endif
...
#if DELAY == DELAY_FAIR_FULL
      while (counter_check && counter_prev == shared_counter)
	{
	  asm volatile ("mfence");
	  /* cpause(32); */
	  if (stop)
...
#if DELAY == DELAY_FAIR_FUT
  size_t delay_expected = (num_threads - 1) * (acq_duration + 101);
  if (delay_expected < 2000)
    {
      fair_delay = 0;
    }
  else if (delay_expected < 56000)
    {
      fair_delay = 8000;
    }
  else
