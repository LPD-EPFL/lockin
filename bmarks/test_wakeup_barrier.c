...
void barrier_clear(barrier_t *b)
{
  pthread_mutex_lock(&b->mutex);
  /* One more thread through */
  b->crossing = INT_MAX;
...
static inline 
uint8_t swap_uint8(volatile uint8_t* target,  uint8_t x) 
{
...
static void
upmutex_lock(upmutex_lock_t* m, thread_data_t* d)
{
...
static void
upmutex_unlock(upmutex_lock_t* m, thread_data_t* d)
{
  volatile ticks s = getticks();
  sys_futex(&m->lock, FUTEX_WAKE_PRIVATE, INT_MAX, NULL, NULL, 0);
  volatile ticks e = getticks();
  d->n_unlock++;
  d->ticks_unlock += (e - s);
}
...
main(int argc, char **argv)
{
  ...
  printf("#avg sleep lo : %-10zu (%zu times) -- per thread: %zu -- w/o delay: %zu\n", 
	 ticks_lock / n_lock, n_lock, ticks_lock / (n_lock * (num_threads - 1)), ticks_lock / (n_lock * (num_threads - 1)) - ticks_cdelay / n_cdelay);
