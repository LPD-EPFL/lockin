...
#define TAS_MWAIT_LAT 0

#include "rapl_read.h"
#include "ttas_mwait_in.h"
...
test(void *data)
{
  ...
  while (stop == 0) 
    {
      tas_mwait_lock_lock(locks + lock_to_acq, fp, lock_to_acq, &lat);
      d->num_acquires++;
    }

  __sync_fetch_and_add(&num_done, 1);
...
main(int argc, char **argv)
{
  ...
  int hw_ctx_n = hw_ctx_nr_get();
  set_cpu(the_cores[hw_ctx_n - 1]);
  ...
  while(1) 
    {
      i = 0;
      c = getopt_long(argc, argv, "hvd:n:s:l:w:a:p:c:o:f:", long_options, &i);
...
#  endif
  ...
  for (l = 0; l < num_locks; l++)
    {
      tas_mwait_lock_init(locks + l, NULL);
    }
