...
#define DEFAULT_CHAGE_PHASE 0.000001
...
test(void *data)
{
  ...
  size_t change_phase = -1;
  change_phase *= change_phase_chance;
  size_t acqd = acq_duration;
  ...
  while (stop == 0) 
    {
      ...
      int lock_to_acq = (int) rand  & rand_max;

      pthread_mutex_lock(locks + lock_to_acq);      

      if (rand < change_phase)
	{
	  uint8_t phase_multi = my_random(&(seeds[0]),&(seeds[1]),&(seeds[2]));
	  acq_duration = phase_multi * acqd;
	  printf("acq dur: %d\n", acq_duration);
	}
...
main(int argc, char **argv)
{
  struct option long_options[] = 
    {
      ...
      {"phase",                     required_argument, NULL, 'e'},
      {NULL, 0, NULL, 0}
    };
  ...
  change_phase_chance = DEFAULT_CHAGE_PHASE;
  ...
  while(1) 
    {
      i = 0;
      c = getopt_long(argc, argv, "hvd:n:s:l:w:a:p:c:o:f:e:", long_options, &i);
	...
	case 'h':
	  printf("lock stress test\n"
		 ...
		 "  -e, --phase <float>\n"
		 "        Chance (%%) to change lenght of CS (default=" XSTR(DEFAULT_CHAGE_PHASE) ")\n"
		 );
	  exit(0);
	case 'v':
	  verbose = 1;
...
#  endif
	...
	case 'o':
	  ...
	  break;
	case 'e':
	  change_phase_chance = atof(optarg);
	  break;
	case '?':
