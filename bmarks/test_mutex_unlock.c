...
_lock_t lock = PTHREAD_MUTEX_INITIALIZER;
...
test(void *data)
{
  ...
  while (1)
      ...
      if (d->id == 0)
	{
	  ...
	  while (*(volatile int*) &lock.__data.__lock != 2)
	    {
	      cpause(3333);
	    }
	  if (acq_duration)
...
main(int argc, char **argv)
{
  ...
  barrier_t* barrier_thr = malloc(sizeof(barrier_t*));
  assert(barrier != NULL && barrier_thr != NULL);
